...
import { ToolListPage as ShowCaseList } from 'pages/tools';
...
function DashboardPage() {
  ...
  const rentalRequestStat = useSWR<PaginatedInterface<RentalRequestInterface>>(
    () => `/rental-requests`,
    () => getRentalRequests({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: rentalRequestStat?.data?.totalCount,
      isLoading: rentalRequestStat.isLoading,
    },
    {
      title: 'Tools',
